...
export async function updateLeadScore(leadId, scoreChange, reason) {
...
export async function appendInsight(leadId, insight) {
  const lead = await get('SELECT ia_insight FROM leads WHERE id = @id', { id: leadId });
  if (!lead) return;

  const existing = (lead.ia_insight || '').split(';').map(s => s.trim()).filter(Boolean);
  const updated = [...new Set([...existing, insight])].slice(-20).join('; ');

  await run("UPDATE leads SET ia_insight = @insight, updated_at = NOW() WHERE id = @id", { insight: updated, id: leadId });
}
...
export async function regenerateSummary(leadId) {
  ...
  const conversation = await buildConversationContext(leadId, 20);
...
Últimas interacciones: ${conversation.slice(-5).map(m => m.content).join(' | ')}`;
      ...
      await run("UPDATE leads SET ia_summary = @summary, updated_at = NOW() WHERE id = @id", { summary: text, id: leadId });
      return text;
    } catch (err) {
  ...
  await run("UPDATE leads SET ia_summary = @summary, updated_at = NOW() WHERE id = @id", { summary: fallback, id: leadId });
  return fallback;
}
