...
router.get('/:leadId/preferences', async (req, res) => {
  try {
    const { leadId } = req.params;
    const agencyId = req.user.agency_id;

    const lead = await get('SELECT id FROM leads WHERE id = @id AND agency_id = @aid', { id: leadId, aid: agencyId });
...
router.patch('/:leadId/preferences', async (req, res) => {
  try {
    const { leadId } = req.params;
    const agencyId = req.user.agency_id;
